...
class stps(models.Model):
    ...
    def create(self, values):
        # Add code here
        URL = 'http://${hostname -I | cut -d ' ' -f 1}:3000/statisticalsector'
    ...
    def write(self, values):
        URL = 'http://${hostname -I | cut -d ' ' -f 1}:3000/statisticalsector/%s' % (self._id)
        new_vals = {
            "year": self.year,
            "month": self.month,
            "salary": self.salary,
    ...
    def unlink(self):
        ...
        for record in self:
            URL = 'http://${hostname -I | cut -d ' ' -f 1}:3000/statisticalsector/%s' % (record._id)
